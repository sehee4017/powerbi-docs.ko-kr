...
## <a name="refresh-your-data"></a>데이터 새로 고침

최신 데이터를 확실히 볼 수 없는 경우 새 데이터를 Power BI 서비스에서 보고서로 끌어올 수 있습니다.

* iOS 디바이스 및 Android 태블릿의 경우 보고서 페이지의 위쪽에서 아래쪽으로 약간 아래로 끌어옵니다.
* Android 휴대폰에서는 [상호 작용 설정](mobile-app-interaction-settings.md)에서 설정한 방법에 따라 풀 다운 작업 또는 새로 고침 단추를 사용할 수 있습니다.
* Windows 디바이스에서는 디스플레이 오른쪽 위에 있는 새로 고침 단추를 사용합니다.

    [자동 페이지 새로 고침](../../create-reports/desktop-automatic-page-refresh.md)이 있는 보고서 페이지는 구성된 대로 자동으로 새로 고쳐집니다(Windows 앱에만 해당).

>[!NOTE]
>위의 새로 고침 메서드는 기본 데이터 세트를 새로 고치지 않습니다. 대신, Power BI에 있을 수 있는 새 데이터를 사용하여 모바일 앱에서 보고 있는 보고서를 업데이트합니다.

### <a name="how-do-i-know-when-my-report-was-last-refreshed"></a>보고서를 마지막으로 새로 고친 시기를 어떻게 알 수 있나요?

보고서를 마지막으로 새로 고친 시기를 확인하려면 보고서 헤더를 탭합니다. 마지막으로 새로 고친 날짜와 시간을 포함하여 보고서의 탐색 트리가 표시됩니다. 

![모바일 앱에 대한 보고서 새로 고침 정보 스크린샷.](media/mobile-reports-in-the-mobile-apps/power-bi-mobile-report-refresh-info.png)
