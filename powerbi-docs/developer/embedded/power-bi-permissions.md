...
| 모든 대시보드 읽기 및 쓰기 | 앱은 로그인한 사용자에 대한 모든 대시보드와 사용자에게 액세스 권한이 있는 모든 대시보드를 보고 편집할 수 있습니다. | Dashboard.ReadWrite.All |
...
| 테넌트의 모든 콘텐츠 보기 |로그인한 사용자가 Power BI 서비스 관리자인 경우 앱은 Power BI에서 그룹, 보고서, 대시보드, 데이터 세트를 포함한 모든 아티팩트를 보고 아티팩트에 쓸 수 있습니다. |Tenant.Read.All |
| 모든 작업 영역 읽기 및 쓰기 | 앱은 로그인한 사용자가 액세스할 수 있는 모든 작업 영역을 보고 편집할 수 있습니다. | Workspace.ReadWrite.All |
...
* **권한 부여 토큰** - 다른 사용자를 대신하여 API 호출을 인증하는 데 사용됩니다. 이 토큰의 범위는 특정 애플리케이션으로 지정됩니다. 토큰에는 설정된 수명이 있으며 토큰이 만료되는 경우 새로 고쳐야 합니다.
